...
import { getFirestore, collection, doc, getDocs, query, where , documentId, limit} from "https://www.gstatic.com/firebasejs/10.8.1/firebase-firestore.js";
...
async function fetchEcgReadingsForDate(userId, date, startTime, endTime) {
...
async function onLoadDataButtonClick(){
  ...
  var pacientNameValue= document.getElementById("pacientName").value;
  ...
  if(!pacientNameValue){
    ...
    if(userId){
      ...
      allStructuredReadings = await fetchEcgReadingsForDate(userId,dateValue,startTimeValue,endTimeValue);
